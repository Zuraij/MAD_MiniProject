...
//                }
                ...
                if (emailEditText.getText().toString().trim().matches(emailPattern)) {
                    dbManager.insert(username, email, firstname, lastname, position, address, gender, Integer.parseInt(phone), Double.parseDouble(salary), password);
                    Toasty.success(getApplicationContext(), "Successfully inserted", Toast.LENGTH_SHORT, true).show();


                } else {
                    Toasty.error(getApplicationContext(), "Not inserted Invalid email address", Toast.LENGTH_LONG, true).show();

                    //Toast.makeText(getApplicationContext()," Not inserted Invalid email address ", Toast.LENGTH_LONG).show();
                }
